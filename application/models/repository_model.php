...
class repository_model extends CI_Model
{
    ...
    public function rulesRepository()
    {
        return [
            [
                'field' => 'link',
    ...
    public function getAll()
    {
        ...
        $sql = "SELECT v_lppm_universitas.id AS id_repository,
        v_lppm_universitas.cek AS cek,
        fakultas.nama AS nama_fak,
        prodi.nama AS nama_prodi,
        r_lppm_universitas.nama AS nama_lppm,
        v_lppm_universitas.link AS link
        FROM
        v_lppm_universitas
        INNER JOIN
        prodi
        ON v_lppm_universitas.prodi = prodi.id
        INNER JOIN
        fakultas
        ON v_lppm_universitas.fakultas = fakultas.id
        INNER JOIN
        r_lppm_universitas
        ON v_lppm_universitas.lppm_universitas = r_lppm_universitas.id
        ORDER BY nama_lppm ASC";
    ...
    public function getByNomor($no)
    {
        ...
        $this->db->select('v_lppm_universitas.id AS id_repository,
        v_lppm_universitas.cek AS cek,
        fakultas.nama AS nama_fak,
        prodi.nama AS nama_prodi,
        r_lppm_universitas.nama AS nama_lppm,
        v_lppm_universitas.link AS link');
        $this->db->from('v_lppm_universitas');
        $this->db->join('prodi', 'v_lppm_universitas.prodi = prodi.id');
        $this->db->join('fakultas', 'v_lppm_universitas.fakultas = fakultas.id');
        $this->db->join('r_lppm_universitas', 'v_lppm_universitas.lppm_universitas = r_lppm_universitas.id');
        $this->db->where('v_lppm_universitas.id', $no);
        $this->db->order_by('nama_lppm', 'ASC');
        $query = $this->db->get();
        return $query->row();
    ...
    public function getRepositoryTersedia(){
        ...
        r_lppm_universitas
        ON v_lppm_universitas.lppm_universitas = r_lppm_universitas.id
        WHERE v_lppm_universitas.cek =0
        ORDER BY nama_lppm ASC";
        return $this->db->query($sql)->result();
    ...
    public function getRepositorybelumTersedia(){
        ...
        r_lppm_universitas
        ON v_lppm_universitas.lppm_universitas = r_lppm_universitas.id
        WHERE v_lppm_universitas.cek =1
        ORDER BY nama_lppm ASC";
        return $this->db->query($sql)->result();
    ...
    public function update()
    {
        ...
        $this->db->where('id', $post['id_repository']);
        $this->db->update('v_lppm_universitas', $data);
    }
