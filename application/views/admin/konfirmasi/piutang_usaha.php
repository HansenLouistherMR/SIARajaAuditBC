...
<table border="1" id="" class="display table table-striped table-bordered" style="width:100%">
    <thead>
        ...
        <tr>
            <th bgcolor="#1471cf" colspan="2" style="color: white;">
                Minimal Konfirmasi 80% - SOP
            </th>
            <th style="color: black;">
            <?php
            $this->load->helper('rupiah_helper');
            $adjusted_rincian = $total_rincian * 80; // If you want to multiply by 100
    ...
    <tbody>
        ...
        <?php $j = 1; ?>
        <?php foreach ($Het_data_piutang_usaha as $Het_data_piutang_usaha_data): ?>
            <tr>
                <td align="center"><?php echo $j ?></td>
                <td align="left" class="editable" >
                    <?php echo $Het_data_piutang_usaha_data->nama_rinvian ?>
                </td>


                <!-- <td style="text-align: center;">
                    <?php echo $Het_data_piutang_usaha_data->account ?>
                </td> -->
                <td style="text-align: right;">
                    <?php
                    $this->load->helper('rupiah_helper');
                    echo rupiah($Het_data_piutang_usaha_data->rincian) ?>
                </td>
                <!-- <td style="text-align: right;">
                    <?php echo $Het_data_piutang_usaha_data->Mata_uang_asing ?>
                </td> -->
                <!-- <td style="text-align: right;">
                    <?php
                    $this->load->helper('rupiah_helper');
                    echo rupiah($Het_data_piutang_usaha_data->Rekening_Koran) ?>
                </td> -->
                <!-- <td style="text-align: right;">
                    <?php
                    // Pastikan nilai yang diambil adalah numerik
                    $rincian = floatval($Het_data_piutang_usaha_data->rincian);
                    $Rekening_Koran = floatval($Het_data_piutang_usaha_data->Rekening_Koran);
                    $this->load->helper('rupiah_helper');
                ...
                <td align="left">
                    <?php echo $Het_data_piutang_usaha_data->arsip ?>
                </td>
                <td style="text-align: right;">
                    <?php 
                     $this->load->helper('rupiah_helper');
                    echo rupiah($Het_data_piutang_usaha_data->Balasan) ?>
                </td>
                <td style="text-align: right;">

                <?php
                    // Pastikan nilai yang diambil adalah numerik
                    $rincian_balasan = floatval($Het_data_piutang_usaha_data->rincian);
                    $Balasan = floatval($Het_data_piutang_usaha_data->Balasan);
                    $this->load->helper('rupiah_helper');

                    // Lakukan pengurangan
                    $hasil_pengurangan_balasan =  intval($Balasan) - intval($rincian_balasan);
                ...
                <td align="left"><?php echo $Het_data_piutang_usaha_data->Keterangan ?></td>
                <td style="text-align: center;">
                <a href="<?php echo site_url('admin/Data_perusahaan/edit/'.$Het_data_piutang_usaha_data->id ) ?>" style="margin-right: 10px"><i class="fa fa-edit"></i> Edit</a>
                </td>
